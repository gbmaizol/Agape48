...
// "This calculator is somebody else's just now." - the contents, with no frame
// around them, so that a desktop can put a window round it and a phone a page.
//
// SPLIT ON 2026sep09, and only because the thing it warns about became possible
// that night. Main.qml used to say so itself: "Somebody else has this
// calculator is still a Window, so on a phone SHOWING it would take the process
// down - the same abort Settings had. It is the last one left, and the least
// reachable: it needs a calculator held by another instance, which cannot
...
Item {
    ...
    // ONE OF THESE TWO IS A DECOY. Main.qml builds both shells on every
    // platform - a Window costs nothing until it is shown - but the engine
    // signals below make this one show ITSELF, so the copy that is not the
    // platform's answer has to be deaf. Without this, sleepUnanswered on a
    // phone would raise the desktop window, which is the crash the split was
    // made to avoid.
    ...
    signal changeFolder()
    // The frame closes itself; the contents never do.
    signal closeRequested()
    // Something happened that the user has to see - a wait that ended in
    // silence. The frame shows and raises itself.
    ...
    Connections {
        target: root.live ? root.engine : null
        function onWaitingChanged() {
            if (root.engine.waitingForCalculator())
                root.phase = "waiting"
        }
        function onOtherLetGo(name) { root.closeRequested() }
        ...
        Column {
            ...
            Label {
                ...
                text: {
                    ...
                    // %n, not %1: "last heard from 1 minutes ago" is what the
                    // phone said the first time this page was ever shown on
                    // one, and a dialog that cannot count to one is not a
                    // dialog anybody trusts about a calculator.
                    return qsTr("It is open on %1, last heard from %n minute(s) ago. Send a sleep command to the other one and take it over?", "", root.quiet)
                           .arg(root.who)
                }
            }
            ...
            Flow {
                ...
                Button {
                    ...
                    onClicked: { root.engine.askForCalculator(root.calculator, false); root.closeRequested() }
                }
                Button {
                    text: qsTr("Choose another…")
                    onClicked: { root.closeRequested(); root.pickAnother() }
                }
                Button {
                    text: qsTr("Another folder…")
                    onClicked: { root.closeRequested(); root.changeFolder() }
                }
                Button { text: qsTr("Cancel"); onClicked: root.closeRequested() }
            }
            ...
            Flow {
                ...
                Button {
                    text: qsTr("Choose another…")
                    onClicked: { root.engine.stopWaiting(); root.closeRequested(); root.pickAnother() }
                }
                Button {
                    text: qsTr("Another folder…")
                    onClicked: { root.engine.stopWaiting(); root.closeRequested(); root.changeFolder() }
                }
                Button {
                    text: qsTr("Stop waiting")
                    onClicked: { root.engine.stopWaiting(); root.closeRequested() }
                }
            }
            ...
            Flow {
                ...
                Button {
                    ...
                    onClicked: if (root.engine.takeOverCalculator(root.calculator)) root.closeRequested()
                }
                Button {
                    text: qsTr("Ask again")
                    onClicked: root.engine.askForCalculator(root.calculator, true)
