...
#endif
{
  ...
  *size = 0;
  ...
  else if (four[1] == 0x49)   /* agape48: was '=', so this branch swallowed
                              * every unrecognised file and sized it at 2x */
    {
  ...
  /* agape48: the four[0] catch-all above accepts any file whose first byte is
   * non-zero as a raw dump, so a text file or a JPEG used to be loaded and then
   * executed as Saturn code. Size cannot reject a real ROM - the smallest HP 48
   * ROM is the SX at 256 KB packed, which is 524288 nibbles - and it rejects
   * everything that is obviously not one, including a malloc of a huge file. */
  if (*size < 524288 || *size > 8 * 1024 * 1024)
    {
      LOGE( "%s is %d nibbles, not a plausible HP 48 ROM\n", name, *size);
      *size = 0;
      fclose(fp);
      return 0;
    }
