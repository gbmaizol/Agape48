...
#endif
{
  ...
  /* Agape48: the one edit in this file. The shim runs on the GUI thread and
   * cannot block, so blockConditionVariable() clears exit_state and this
   * function returns after a single wake-check pass whether or not anything
   * actually woke. The caller has to be able to tell those two apart - see the
   * note in x48_shim.c - or a sleeping calculator makes spurious progress ten
   * times a second and eventually switches its own screen back on. */
  extern int x48_shutdn_woke;
  ...
  x48_shutdn_woke = wake;               /* Agape48, see above */
