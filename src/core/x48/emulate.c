...
/* ---------------------------------------------------------------------------
 * Agape48: bounded entry points.
 *
 * emulate() above owns its own timing - it loops until told to stop, and its
 * throttle busy-waits ~2us per instruction. Agape48 drives from a Qt timer on
 * the GUI thread instead, so it needs the prologue once and the body N times.
 * Both live here rather than in the shim so they can see this file's statics.
 * emulate() itself is left untouched and simply goes uncalled.
 * ------------------------------------------------------------------------ */

extern int GetEvent(void);

void
agape48_emulate_begin(void)
{
  reset_timer(T1_TIMER);
  reset_timer(RUN_TIMER);
  reset_timer(IDLE_TIMER);
...
int
agape48_emulate_slice(int max_instructions)
{
  int n = 0;

  /* schedule() pumps GetEvent() only under got_alarm (line 2421), which nothing
   * raises in this build, and the SIGALRM loop that used to raise it lived in
   * the frontend we do not compile. So pump it once per slice: that is the only
   * thing that carries a key pressed between slices into the Saturn.
   *
   * Except while the machine is parked on a SHUTDN, and that exception is the
   * whole of why Esc would not switch a sleeping calculator back on. Delivering
   * the key here jumps the Saturn to the interrupt handler at 0xf, which
   * returns to the SHUTDN and parks again; the ROM's own wake-up code, the part
   * that turns the display back on, lives AFTER the SHUTDN and is reached only
   * when do_shutdown() decides it has a reason to wake. Pumping here stole the
   * key before do_shutdown's wake loop could see it, so wake stayed 0 for ever.
   * Parked, we execute the SHUTDN instead and let its own pass have the key.
   * See the GetEvent() note in x48_shim.c. */
  {
    extern int x48_parked;
    if (!x48_parked)
      GetEvent();
  }
  ...
  while (n < max_instructions) {
    ...
    /* do_shutdown() returns the moment the shim clears exit_state, which is
     * how a sleeping calculator hands the GUI thread back. */
    if (!exit_state)
      break;
  }
